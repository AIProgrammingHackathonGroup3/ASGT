...
public class Matcher {
	...
	static void match(boolean allowPartial, int i, int count,
			List<Predicate> ps, List<Predicate> qs, Bind b, Collection<Bind> unifiers) {
		if (i >= ps.size()) {
			if (isGround(ps) && count > 0)
				unifiers.add(b);
			else if (count > 0 && b.vars.size() > 0)
				unifiers.add(b);
		} else {
			var pf = b.instantiate(ps.get(i));
			for (var pg : qs) {
				var b1 = b.unified(pf, pg);
				if (b1.isSatisfied() == false)
					continue;
				match(allowPartial, i + 1, count + 1, ps, qs, b1, unifiers);
			}

			if (allowPartial)
				match(allowPartial, i + 1, count, ps, qs, b, unifiers);
		}
	}
}
